...
test("LRTS 迁移 fixture：分类菜单与播放 JSON 走通用计划，无域名分支", async () => {
  ...
  // Pagination must stay on the chapters adapter; raw getBookMenu has data:null.
  ...
  // Same leftover-detail bug would send getBookDetail into /adapter/media.
  const lrtsRequestJs = String(converted.chapterContent.requestInfo).replace(/^@js:\s*/i, "");
  const lrtsBuild = new Function("config", "params", "result", lrtsRequestJs);
  const lrtsDetail = "https://m.lrts.me/ajax/getBookDetail?bookId=5622";
  const lrtsChapter = "https://m.lrts.me/ajax/getListenPath?entityId=5622&section=1&id=9";
  const lrtsLeftover = lrtsBuild(
    { host: "https://m.lrts.me", httpHeaders: converted.httpHeaders },
    { queryInfo: { detailUrl: lrtsDetail, url: lrtsDetail }, pageIndex: 1 },
    lrtsChapter,
  );
  assert.match(decodeURIComponent(lrtsLeftover.split("&url=").pop()), /getListenPath/);
  assert.doesNotMatch(decodeURIComponent(lrtsLeftover.split("&url=").pop()), /getBookDetail/);
