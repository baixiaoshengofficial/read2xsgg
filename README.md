...
`1197.json` 这类聚合源包含小说、漫画、音频、视频和文件下载源。当前版本已用该地址进行整包转换验证：828 个条目均可生成 XBS 且带有可选分类，能力推断结果为 723 个文本源、86 个漫画源、16 个音频源和 3 个视频源。这个数字表示转换结构完整，不代表上游 828 个网站都仍然在线；连接失败、登录限制或规则已过期的站点会保留告警并跳过在线验证。
